...
export const createBoard = async (req: Request, res: Response) => {
  ...
  const bodyError = checkBody(req.body, ['title', 'owner', 'users', 'description', 'color'])
  if (bodyError) {
    return res.status(400).send(createError(400, "bad request: " + bodyError));
  }

  const { title, owner, users, description, color  } = req.body;
  try {
    const newBoard = await boardService.createBoard({ title, owner, users, description, color }, guid, initUser);
    res.json(newBoard);
  }
...
export const updateBoard = async (req: Request, res: Response) => {
  ...
  try {
    const updatedBoard = await boardService.updateBoard(req.params['boardId'], { title, owner, users, description, color }, guid, initUser);
